...
export function createRepositoryTools({ config, gitlab }) {
    return [
        defineTool("get_repository_file", "Read one repository file with size limits.", { projectPath, filePath: z.string().min(1), ref: z.string().min(1) }, async (args) => {
        ...
        defineTool("list_repository_tree", "List repository tree entries.", { projectPath, ref: z.string().min(1), path: optionalString, recursive: z.boolean().optional() }, async (args) => {
            assertAllowed(config, { projectPath: args.projectPath, tool: "list_repository_tree" });
            return json(await gitlab.listRepositoryTree(args.projectPath, {
                ref: args.ref,
                path: args.path,
