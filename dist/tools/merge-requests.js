...
export function createMergeRequestTools({ config, gitlab }) {
    return [
        defineTool("list_merge_requests", "List merge requests.", {
            projectPath,
            ...
            return json(await gitlab.listMergeRequests(args.projectPath, {
                state: args.state,
        ...
        defineTool("create_merge_request", "Create a merge request without merge or approval actions.", {
            projectPath,
        ...
        defineTool("comment_on_merge_request", "Add a comment to a merge request.", { projectPath, mergeRequestIid: id(), body: z.string().min(1) }, async (args) => {
